...
public class MusicCollectionServiceImpl implements MusicCollectionService {
    ...
    private void checkOwner(final MusicCollection collection) {
        ...
        if (!currentUserName.equals(owner.getUserName())) {
            //TODO Access denied and check for admin
            throw new RuntimeException(format("User {0} hasn't access to collection with name {1}",
